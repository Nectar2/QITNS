...
%PRESSURE M-file for Pressure.fig
...
%
%      PRESSURE('Property','Value',...) creates a new PRESSURE using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Pressure_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      PRESSURE('CALLBACK') and PRESSURE('CALLBACK',hObject,...) call the
%      local function named CALLBACK in PRESSURE.M with the given input
%      arguments.
%
...
% Last Modified by GUIDE v2.5 19-Jan-2024 20:24:15
...
global WAC;
        ...
        Ttest=[0:Tfs:1000]/(1e6);   %calculation time
        Odata=[0.1*rr 0];    %Initial position and speed
        ...
        fft1=abs(fft(Rout));
        FFT1=fft1/max(fft1);
        w1=(0:1000/Tfs/N1:(N1-1)/N1*1000/Tfs);  %unit(kHz)
        axes(handles.axes2);
